...
import {
  ...
  dambukHeroImages,
  dambukMeta,
} from "@/data/places/dambuk";
import { createTripPlanningURL } from "@/lib/whatsapp";

export function DambukHero() {
  const {
    props: { srcSet: desktopSrcSet },
  } = getImageProps({
    src: dambukHeroImages.desktop.src,
    alt: dambukHeroImages.alt,
    width: dambukHeroImages.desktop.width,
    height: dambukHeroImages.desktop.height,
    quality: 85,
    sizes: "100vw",
    priority: true,
  });
  ...
  const { props: mobileImageProps } = getImageProps({
    src: dambukHeroImages.mobile.src,
    alt: dambukHeroImages.alt,
    width: dambukHeroImages.mobile.width,
    height: dambukHeroImages.mobile.height,
    quality: 85,
    sizes: "100vw",
    priority: true,
  });
  ...
  return (
    <section
      aria-labelledby="dambuk-hero-title"
    ...
    >
      ...
      <div className="absolute left-5 top-24 z-10 sm:left-8 md:left-10 md:top-28 lg:left-16 xl:left-24">
        <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#e7d9b8]">
          {dambukMeta.region} · {dambukMeta.state}
        </p>
      </div>
        ...
        <div className="flex flex-1 items-end pb-7 md:pb-10">
          <div className="max-w-[980px]">
            ...
            <p className="mt-5 max-w-[38rem] text-[0.95rem] leading-6 text-white/[0.74] sm:mt-6 sm:text-base sm:leading-7 md:text-lg md:leading-8">
              On the banks of one of India&apos;s last undammed big rivers — a town
              the ferry era cut off for decades, now famous for the sweetest oranges
              in the Northeast and sunsets that turn the shingle gold.
            </p>
...
export function DambukExperiences() {
  return (
    <section
      id="dambuk-experiences"
      aria-labelledby="dambuk-experiences-title"
    ...
    >
      <div className="mx-auto w-full max-w-[1600px] px-5 sm:px-8 md:px-10 lg:px-16 xl:px-24">
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:mt-16">
          {dambukExperiences.map((experience) => (
            <article
