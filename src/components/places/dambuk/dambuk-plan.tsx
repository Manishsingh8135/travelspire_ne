...
import { dambukFrames, dambukNextPlaces, dambukSeasons } from "@/data/places/dambuk";
...
export function DambukFrames() {
  return (
    <section aria-labelledby="dambuk-frames-title" className="bg-[#050d0f] py-20 text-white sm:py-24 lg:py-32">
...
export function DambukSeasons() {
  return (
    <section aria-labelledby="dambuk-seasons-title" className="bg-[#f1ebdf] py-20 text-[#17221b] sm:py-24 lg:py-28">
      <div className="mx-auto w-full max-w-[1600px] px-5 sm:px-8 md:px-10 lg:px-16 xl:px-24">
        <div className="max-w-2xl">
          ...
          <h2
            id="dambuk-seasons-title"
            className="text-[clamp(2.75rem,6vw,4.5rem)] font-medium leading-[0.94] tracking-[-0.055em]"
          >
            When Dambuk is{" "}
            <span className="font-serif font-normal italic text-[#76533e]">at its best</span>
          </h2>
        </div>
        ...
        <div className="mt-12 overflow-hidden rounded-[16px] border border-[#d5c9ae] bg-[#faf7f0] lg:mt-14">
          <ol className="grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-12">
            {dambukSeasons.map((season) => (
              <li
                key={season.month}
                className="group flex min-h-[7.5rem] flex-col justify-between border-[#e2d9c3] p-3.5 odd:border-r sm:min-h-[9rem] sm:border-r lg:[&:nth-child(6)]:border-r"
              >
        ...
        <p className="mt-6 max-w-2xl text-sm leading-6 text-[#5a5344]">
          Dambuk is a flood-plain town — in monsoon the Dibang reclaims the banks and the
          road is a risk, not a route. November to January is when the oranges, the
          weather and the river light all agree.
        </p>
      </div>
    </section>
  );
...
export function DambukGo() {
  return (
    <>
      <section aria-labelledby="dambuk-go-title" className="bg-[#050d0f] py-20 text-white sm:py-24 lg:py-32">
        <div className="mx-auto w-full max-w-[1600px] px-5 sm:px-8 md:px-10 lg:px-16 xl:px-24">
          <div className="mx-auto max-w-3xl text-center">
            ...
            >
              Taste{" "}
              <span className="font-serif font-normal italic text-[#dfcfab]">Dambuk</span>
            </h2>
            <p className="mx-auto mt-6 max-w-xl text-base leading-7 text-white/[0.72]">
              We run this road every week. Orchards, the bridge at dusk, and the long
              road onward to Anini or Dong — tell us your dates and we&apos;ll build the
              whole thing.
            </p>
            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
              <a
                href={createTripPlanningURL({ destination: "Dambuk, Lower Dibang Valley" })}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Plan a Dambuk trip on WhatsApp (opens in a new tab)"
      ...
      <section aria-labelledby="dambuk-next-title" className="border-t border-white/[0.08] bg-[#050d0f] py-20 text-white sm:py-24">
        <div className="mx-auto w-full max-w-[1600px] px-5 sm:px-8 md:px-10 lg:px-16 xl:px-24">
          <div className="mb-12 flex flex-wrap items-end justify-between gap-6">
            <h2
              id="dambuk-next-title"
